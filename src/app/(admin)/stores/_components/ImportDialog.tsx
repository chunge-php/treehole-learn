...
import { Download, FileSpreadsheet, Loader2, CheckCircle2, AlertTriangle, X, RotateCcw, Upload } from "lucide-react";
...
export function ImportDialog({ open, onOpenChange, onDone }: { open: boolean; onOpenChange: (v: boolean) => void; onDone?: () => void }) {
  ...
  const [result, setResult] = useState<{ total: number; success: number; failed: number; errors: { row: number; message: string }[] } | null>(null);
  ...
  function upload() {
    ...
    start(async () => {
      try {
        const r = await bulkImportStores(rows);
        setResult(r);
        setStage("done");
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            文件在浏览器本地解析，不会被上传到服务器；点击「开始导入」后才会写入数据库。
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          {stage === "importing" && (
            <div className="rounded-xl border bg-card p-4 space-y-3">
              ...
              {/* 跑马灯进度条 (无法预知精确进度,但有视觉反馈) */}
              <div className="h-1.5 overflow-hidden rounded-full bg-muted">
                <div className="h-full w-1/3 rounded-full bg-gradient-to-r from-primary via-info to-primary animate-[slide_1.2s_ease-in-out_infinite]" style={{
